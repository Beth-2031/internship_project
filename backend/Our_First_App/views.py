...
class EvaluationViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        if self.request.user.user_type not in ['workplace_supervisor','academic_supervisor','internship_admin']:
            raise permissions.PermissionDenied("Only supervisors can create evaluations.")
        serializer.save()
...
def academic_dashboard(request):
    ...
    logs = WeeklyLog.objects.filter(placement__academic_supervisor=user).select_related('student', 'placement')
    return JsonResponse({
        'total_placements': placements.count(),
        'total_logs': logs.count(),
        'verified_logs': logs.filter(is_verified=True).count(),
...
def admin_dashboard(request):
    ...
    return JsonResponse({
        ...
        'approved_placements': InternshipPlacement.objects.filter(is_approved=True).count(),
        'pending_placements': InternshipPlacement.objects.filter(is_approved=False).count(),
        'total_logs': WeeklyLog.objects.count(),
        'verified_logs': WeeklyLog.objects.filter(is_verified=True).count(),
        'pending_review': SupervisorReview.objects.filter(status='pending').count(),
        'total_safety_reports': SafetyReport.objects.count(),
        'unresolved_reports': SafetyReport.objects.filter(is_resolved=False).count(),
...
def submit_log(request, log_id):
    ...
    if log.student != request.user:
        return JsonResponse({'error': 'This log is locked and cannot be submitted.'}, status=403)
...
def edit_weekly_log(request, log_id):
    log = get_object_or_404(WeeklyLog, id=log_id)
    if log.is_verified:
        return JsonResponse({'error': 'This log has been verified and cannot be edited.'}, status=403)
    if log.student != request.user:
...
def edit_placement(request, placement_id):
    ...
    if placement.is_approved:
        return JsonResponse({'error': 'This placement has been approved and cannot be edited'}, status=403)
    if request.method == 'POST':
